...
public class CoinChangeProblem {
  ...
  static int noOfWaysToChangeCoinsDP(int arr[], int n, int sum) {
    ...
    for (int i = 1; i <= n; i++) {
      for (int j = 1; j <= sum; j++) {
        ...
        if (arr[i - 1] <= j) {
          // we have two choice either include or exclude, here wer are calculating numbers of ways, so sum it up
  ...
  public static void main(String[] args) {
    ...
    System.out.println(noOfWaysToChangeCoinsDP(arr, arr.length, 4));
